...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^elixir_") ; then
        ELIXIR_BUILD=$(echo -n $1 | sed -e 's/^elixir_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ELIXIR_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ELIXIR_BUILD=
    fi
    export ELIXIR_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the ElixirOS GitHub
        T=$(gettop)
        cd $T > /dev/null
